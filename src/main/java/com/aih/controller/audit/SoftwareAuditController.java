...
public class SoftwareAuditController {
     ...
     */
    @ApiOperation(value = "[教师]提交软件著作审核")
    @PostMapping("/submit")
    public R<Long> saveSoftwareAudit(@RequestBody SoftwareAudit software){
        ...
        Long id = software.getId(); // 返回id,用于调用上传附件接口
        return R.success(id);
    }
     ...
     */
    @ApiOperation(value = "根据软件著作id查询具体信息")
    @GetMapping("/query/{id}")
    public R<SoftwareVo> queryById(@PathVariable("id") Long id) {
        return R.success(softwareService.queryDtoById(id)); //权限在service里验证
    }

    /**
     * 根据id通过软件著作审核,审核员可添加审核员备注 审核员(aid)&审核时间(updateTime)会自动填充 id不存在/已审核过/没审核权限会抛出自定义异常
     * @param id 记录id
     * @param auditorRemark 审核员备注(可选)
     */
    @ApiOperation(value = "通过软件著作审核")
    @PutMapping("/pass")
    public R<?> passSoftwareAudit(@RequestParam("id") Long id,@RequestParam(value = "auditorRemark",required = false) String auditorRemark){
        //判断id是否存在
        SoftwareAudit findRecord = softwareService.getById(id);
        if (findRecord == null){
        ...
        softwareService.passSoftwareAudit(findRecord);
//        this.makeFilesUsefulByID(id);
        return R.success("审核通过");
    }

    /**
     * 根据id驳回软件著作审核,审核员可添加审核员备注 审核员(aid)&审核时间(updateTime)会自动填充。id不存在/已审核过/没审核权限会抛出自定义异常
     * @param id 记录id
     * @param auditorRemark 审核员备注(可选)
     */
    @ApiOperation(value = "驳回软件著作审核")
    @PutMapping("/reject")
    public R<?> rejectSoftwareAudit(@RequestParam("id") Long id,@RequestParam(value = "auditorRemark",required = false) String auditorRemark){
        //判断id是否存在
        SoftwareAudit findRecord = softwareService.getById(id);
        if (findRecord == null){
            throw new CustomException(CustomExceptionCodeMsg.ID_NOT_EXIST);
        ...
        softwareService.rejectSoftwareAudit(findRecord);
     ...
     * @param pageNum 页码
     * @param pageSize 每页条数
     * @param keyword 模糊查询关键字(可选) 软件著作名称[softwareName]
     */
    @ApiOperation(value = "[教师]查询自己审核记录")
    @GetMapping("/queryOwnRecord")
    public R<Page<SoftwareVo>> queryOwnRecord(@RequestParam(value = "auditStatus",required = false) Integer auditStatus,
        ...
        return R.success(softwareService.queryOwnRecord(pageNum,pageSize,auditStatus, keyword));
    }
    ...
    public R<Page<SoftwareVo>> queryPowerRecords(@RequestParam("pageNum") Integer pageNum,
                                                 @RequestParam("pageSize") Integer pageSize,
        ...
        return R.success(softwareService.queryPowerRecords(pageNum,pageSize,auditStatus,onlyOwn,keyword));
    }
    ...
    public R<?> deleteOwnInfo(@PathVariable("id") Long id){
        softwareService.deleteOwnInfo(id);
    ...
    public R<?> deleteRecord(@PathVariable("id") Long id){
        String data = softwareService.deleteRecord(id);
    ...
    @PostConstruct
    public void init() {
        basePath = rootPath + sep + "software"; // files->software
    }
    ...
    public R<ArrayList<String>> upload(@PathVariable("id") Long id, MultipartFile[] fileList) throws IOException {
        //判断id是否存在
        SoftwareAudit findRecord = this.checkIdExistAndReturn(id);
        ...
        // === 开始 ===
        String parentPath =  basePath + sep + tid + sep + id; // files->software->tid->id
        ...
        for (MultipartFile file : fileList) {
            ...
            String url = "http://" + ip + ":" + port + "/softwareAudit/file/download/"  + id + "/" + fileName;
            urls.add(url);
        }
    ...
    public void export(@PathVariable("id") Long id, HttpServletResponse response) throws IOException {
        ...
        String fileName = teacherName + "-软件著作附件.zip";
     ...
     * @param id  记录id.
     * @param fileName 软件著作附件文件名,全称(带后缀)
     * @param response
     * @throws IOException
     */
    @AuthAccess
    @ApiOperation("下载软件著作附件")
    ...
    public void download(@PathVariable("id")Long id,
        ...
        Long tid = findRecord.getTid();
        String parentPath =  basePath + sep + tid + sep + id; // files->software->tid->id
        String filePath = parentPath + sep + fileName; // files->software->tid->id->fileName
    ...
    public R<?> isExist(@PathVariable("id") Long id, @PathVariable("fileName") String fileName) {
        ...
        Long uid = UserInfoContext.getUser().getId();
        String parentPath =  basePath + sep + uid + sep + id; // files->software->uid->id
    ...
    /**
     * 删除的软件著作文件夹(包含里面所有软件著作文件)。且自己才能删除自己的附件。若不存在则返回文件夹不存在
     */
    @ApiOperation("[教师用户]删除自己的所有软件著作文件夹")
    ...
    /**
     * 根据id和具体文件名删除自己的软件著作附件。如果是最后一个文件,则删除父级文件夹。检验fileName抛出自定义异常。
     * @param id 记录id
     * @param fileName 文件名,全称(带后缀)
     */
    @ApiOperation("[教师用户]删除自己的软件著作附件")
    @DeleteMapping("/file/delete/{id}/{fileName}")
    public R<?> delete(@PathVariable("id") Long id,@PathVariable("fileName") String fileName){
        MyUtil.checkIsTeacher();//判断是否是教师
        Long uid = UserInfoContext.getUser().getId();
        String parentPath =  basePath + sep + uid + sep + id; // files->software->uid->id
        String filePath = parentPath + sep + fileName; // files->software->uid->id->fileName
        if (!FileUtil.exist(filePath)) {//如果不存在
            throw new CustomException(CustomExceptionCodeMsg.FILE_NOT_EXIST);
        }
     ...
     * return true/false
     */
    @ApiOperation("[教师用户]判断自己是否已有软件著作附件")
    @GetMapping("/file/isExist")
    public R<?> isExist() {
        MyUtil.checkIsTeacher();//判断是否是教师
        Long uid = UserInfoContext.getUser().getId();
        String parentPath =  basePath + sep + uid; // files->software->uid
        if (!FileUtil.exist(parentPath)) {//如果不存在
            return R.success(false);
        }
        return R.success(true);
    ...
    private SoftwareAudit checkIdExistAndReturn(Long id) {
        SoftwareAudit findRecord = softwareService.getById(id);
        if (findRecord == null) {
            throw new CustomException(CustomExceptionCodeMsg.ID_NOT_EXIST);
        }
