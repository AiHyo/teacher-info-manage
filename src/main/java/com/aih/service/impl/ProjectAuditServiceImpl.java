...
public class ProjectAuditServiceImpl extends ServiceImpl<ProjectAuditMapper, ProjectAudit> implements IProjectAuditService {
    ...
    @Override
    public void passProjectAudit(ProjectAudit project) {
    ...
    @Override
    public void rejectProjectAudit(ProjectAudit project) {
    ...
    public Page<ProjectVo> queryOwnRecord(Integer pageNum, Integer pageSize, Integer auditStatus, String keyword) {
        ...
        queryWrapper.eq(ProjectAudit::getTid, uid) //查自己的
    ...
    public Page<ProjectVo> queryPowerRecords(Integer pageNum, Integer pageSize, Integer auditStatus, Boolean onlyOwn, String keyword) {
        ...
        LambdaQueryWrapper<ProjectAudit> queryWrapper = Wrappers.lambdaQuery();
        queryWrapper.in(ProjectAudit::getTid, queryTids)
                .eq((auditStatus != null), ProjectAudit::getAuditStatus, auditStatus) //0,1,2
                .ge(ProjectAudit::getCreateTime, UserInfoContext.getUser().getCreateDate().atStartOfDay())//上任以来(针对未审核的)
                .eq(onlyOwn, ProjectAudit::getAid, UserInfoContext.getUser().getId())//只看自己的(针对已审核的)
                .like((StringUtils.isNotBlank(keyword)), ProjectAudit::getProjectName, keyword)//模糊查询
    ...
    public void deleteOwnInfo(Long id) {
        ProjectAudit project = this.checkIdExistAndReturn(id);
        Long uid = UserInfoContext.getUser().getId();
        ...
        LambdaUpdateWrapper<ProjectAudit> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(ProjectAudit::getId, id)
                .set(ProjectAudit::getIsShow, 0);
...
/*        project.setDeleteRoles(deleteRoles);
        ...
        LambdaUpdateWrapper<ProjectAudit> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(ProjectAudit::getId, id)
                .set(ProjectAudit::getDeleteRoles, deleteRoles);
