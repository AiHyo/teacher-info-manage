...
public class SoftwareAuditServiceImpl extends ServiceImpl<SoftwareAuditMapper, SoftwareAudit> implements ISoftwareAuditService {
    ...
    @Override
    public void passSoftwareAudit(SoftwareAudit project) {
    ...
    @Override
    public void rejectSoftwareAudit(SoftwareAudit project) {
    ...
    public Page<SoftwareVo> queryOwnRecord(Integer pageNum, Integer pageSize, Integer auditStatus, String keyword) {
        ...
        queryWrapper.eq(SoftwareAudit::getTid, uid) //查自己的
    ...
    public Page<SoftwareVo> queryPowerRecords(Integer pageNum, Integer pageSize, Integer auditStatus, Boolean onlyOwn, String keyword) {
        ...
        LambdaQueryWrapper<SoftwareAudit> queryWrapper = Wrappers.lambdaQuery();
        queryWrapper.in(SoftwareAudit::getTid, queryTids)
                .eq((auditStatus != null), SoftwareAudit::getAuditStatus, auditStatus) //0,1,2
                .ge(SoftwareAudit::getCreateTime, UserInfoContext.getUser().getCreateDate().atStartOfDay())//上任以来(针对未审核的)
                .eq(onlyOwn, SoftwareAudit::getAid, UserInfoContext.getUser().getId())//只看自己的(针对已审核的)
                .like((StringUtils.isNotBlank(keyword)), SoftwareAudit::getSoftwareName, keyword)//模糊查询
    ...
    public void deleteOwnInfo(Long id) {
        SoftwareAudit project = this.checkIdExistAndReturn(id);
        Long uid = UserInfoContext.getUser().getId();
        ...
        LambdaUpdateWrapper<SoftwareAudit> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(SoftwareAudit::getId, id)
                .set(SoftwareAudit::getIsShow, 0);
...
/*        project.setDeleteRoles(deleteRoles);
        ...
        LambdaUpdateWrapper<SoftwareAudit> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(SoftwareAudit::getId, id)
                .set(SoftwareAudit::getDeleteRoles, deleteRoles);
